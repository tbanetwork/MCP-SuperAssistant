 ...
 * Chat Input Handler
 *
 * Utility functions for interacting with the LangDock chat input area
 */
...
/**
 * Find the LangDock chat input element
 * @returns The chat input element or null if not found
 */
export const findChatInputElement = (): HTMLElement | null => {
  // Try to find LangDock's textarea or contenteditable div
  // Updated selectors for LangDock's DOM structure
  const langdockInput = document.querySelector(
    'textarea[placeholder], div[contenteditable="true"], textarea[spellcheck="false"], textarea[data-gramm="false"]',
  );

  if (langdockInput) {
    logMessage('Found LangDock input element');
    lastFoundInputElement = langdockInput as HTMLElement;
    return langdockInput as HTMLElement;
  }
  ...
  const possibleInputSelectors = [
    ...
    // Add more specific selectors for LangDock
  ...
  for (const selector of possibleInputSelectors) {
    ...
    if (element) {
      logMessage(`Found LangDock input element via selector: ${selector}`);
  ...
  logMessage('Could not find LangDock input element');
  return null;
};
...
export const insertTextToChatInput = (text: string): boolean => {
  try {
    ...
    if (!chatInput) {
      logMessage('Could not find LangDock input element');
      console.error('Could not find LangDock input element');
      return false;
    }
    ...
    logMessage(`Error inserting text into LangDock input: ${error}`);
    console.error('Error inserting text into LangDock input:', error);
    return false;
  }
};
...
export const insertToolResultToChatInput = (result: string): boolean => {
  logMessage('Inserting tool result to LangDock chat input');
...
export const attachFileToChatInput = (file: File): boolean => {
  logMessage(`Attempting to attach file to LangDock chat input: ${file.name}`);
  try {
    // Find file input element
    const fileInput = document.querySelector('input[type="file"]') as HTMLInputElement;

    if (!fileInput) {
      logMessage('Could not find file input element in LangDock');
      return false;
    }
    ...
    logMessage(`Error attaching file to LangDock chat: ${error}`);
    console.error('Error attaching file to LangDock chat:', error);
    return false;
  }
};
...
export const submitChatInput = async (maxWaitTime = 5000): Promise<boolean> => {
  try {
    ...
    // Check if we're on LangDock
    if(window.location.hostname === 'app.langdock.com') {
      chatInput = findChatInputElement();
      
      if (!chatInput) {
        logMessage('Could not find LangDock input element for submission');
        return false;
      }
    }
    ...
    return new Promise(resolve => {
      setTimeout(() => {
        ...
        logMessage('Could not find a way to submit the LangDock chat input');
        resolve(false);
      }, 500);
    });
  } catch (error) {
    logMessage(`Error submitting LangDock chat input: ${error}`);
    console.error('Error submitting LangDock chat input:', error);
    return false;
  }
};
